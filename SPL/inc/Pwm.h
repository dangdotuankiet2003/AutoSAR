 ...
 * @brief:   Kiểu dữ liệu đại diện cho số kênh PWM
 * @details: Sử dụng để xác định kênh PWM cụ thể trong driver
 */
typedef uint8 Pwm_ChannelType;

/* @typedef: Pwm_PeriodType
 * @brief:   Kiểu dữ liệu cho chu kỳ PWM
 * @details: Đơn vị là ticks, tùy thuộc vào triển khai phần cứng (ví dụ: timer counter)
 */
typedef uint32 Pwm_PeriodType;

/* @typedef: Pwm_OutputStateType
 * @brief:   Kiểu dữ liệu cho trạng thái đầu ra PWM
 * @details: Xác định mức logic cao hoặc thấp cho đầu ra PWM
 */
typedef enum
{
 ...
 * @brief:   Kiểu dữ liệu cho loại cạnh ngắt PWM
 * @details: Xác định cạnh (lên, xuống hoặc cả hai) để kích hoạt thông báo cho PWM
 */
typedef enum {
 ...
 * @brief:   Kiểu dữ liệu cho lớp kênh PWM
 * @details: Xác định loại kênh (chu kỳ cố định, dịch chuyển, thay đổi)
 */
typedef enum {
...
/* @typedef: Pwm_ConfigType
 * @brief:   Kiểu dữ liệu cấu hình cho PWM driver
 * @details: Cấu trúc chứa dữ liệu khởi tạo phần cứng cụ thể cho PWM
 */
typedef struct {
    TIM_TypeDef* TIMx;                   /* Con trỏ đến timer base (ví dụ: TIM2, TIM3) */
    uint8_t channel;                     /* Số kênh (1-4 trên STM32) */
    Pwm_ChannelClassType channelClass;   /* Lớp kênh (variable/fixed/shifted) */
    Pwm_OutputStateType polarity;        /* Cực tính (PWM_HIGH hoặc PWM_LOW), dùng trong PWM Mode 1 / PWM Mode 2 */
...
/* @typedef: Pwm_ConfigType
 * @brief:   Kiểu dữ liệu cấu hình cho PWM driver
 * @details: Cấu trúc chứa dữ liệu khởi tạo phần cứng của PWM
 */
typedef struct {
    uint8_t numChannels;                     /* Số lượng kênh PWM */
    const Pwm_ChannelConfigType* channels;  /* Mảng cấu hình các kênh */
    /* Thêm nếu cần: Global prescaler, clock source, hoặc các param chung khác cho STM32 */
} Pwm_ConfigType;
                                                 /* FUNCTION DEFINITONS */
